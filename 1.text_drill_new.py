...
space_count = correct_text_word = wrong_text_word = start_times = words_counter = counter = check = start_point = end_point = text_area_start_point = text_area_end_point = text_area_row_no = 0;
type_word = '';
flag = False
flag_st_point=0;
xx = 0;
...
def paragraph_drill_ftn(all_words):
    ...
    Next_word();
    def OOk_fun(event=None):
        left_side_Frame.place_forget();
        right_side_Frame.pack_forget();
        sub_ok_ftn();
    okBtn = Button(right_side_Frame,text="OK",font=("time new roman",15),activebackground=right_side_bg,padx=5,relief="solid",bg="#d2f69f",bd=1,width=10,command=OOk_fun);
    def exit_Function():
        ...
        last_time = gettimes();
        total_time = last_time - start_times;
        Min = Sec = t_m = 0;
        ...
        if(total_time >= 60):
            ...
            Sec = total_time;
        if start_times != 0 and space_count == 1:
            t_m = total_time/60;
            gross = (correct_text_word+wrong_text_word)//t_m;
            net = gross-wrong_text_word//t_m;
    ...
    exit_btn = Button(right_side_Frame,text="Next",font=("time new roman",15),activebackground=right_side_bg,padx=5,relief="solid",bg="#d2f69f",bd=1,width=10,command=exit_Function);
    ...
    def run_time(event=None):
        ...
        if time_Sec == 0 and time_Min == 0:
          exit_Function();
            ...
            time_lable.after(1000,run_time);
    #End Time Part
    def Keypresse(event=None):
        global start_times, space_count, correct_text_word, wrong_text_word, time_run_flag,xx, yy, check, counter, flag_st_point, text_area_end_point, text_area_start_point, text_area_row_no,flag;
        if time_run_flag == False:
            start_times = gettimes();     
            run_time();
        time_run_flag = True; #this part is Flase when after calling this ftn;
        text_area.focus_set()
        char = event.keysym;
        rowed = 1;
        if char == "Return":
